  ...
  if (function)
    {
      ...
      snprintf (buf, min (function_length - 1, 40), "%s", function);
      PUTS (buf);
    }
     ...
     int a, b;
{
  ...
  /* Note: we can have B < A in the case of a range of no lines.
     ...
     which is B.  */
  if (trans_b <= trans_a)
    PRINTF (trans_b == trans_a ? "%d" : "%d,0", trans_b);
  else
    PRINTF ("%d,%d", trans_a, trans_b - trans_a + 1);
}

